...
public class HomePage extends BasePage {
	...
	public List<Float> getPrices() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (priceList.size() > 0) {
				...
				util.log("No Price Details Found ");
			}
		}
	...
	public void comparePrices(List<Float> list) {
		for (int i = 0; i < (list.size() - 1); i++) {
			if (Float.compare((list.get(i)), (list.get(i + 1))) == 0) {
				...
				util.log("For " + i + "---" + (list.get(i)) + " " + (list.get(i + 1)) + " both are equal");
			} else if (Float.compare((list.get(i)), (list.get(i + 1))) < 0) {

				util.log("For " + i + "---" + (list.get(i)) + " " + (list.get(i + 1)) + " is second is greater");
			} else {

				util.log("For " + i + "---" + (list.get(i)) + " " + (list.get(i + 1)) + " is first is greater");
			}
		}
	}
	/*
	...
	public void getProductDetails() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (titleList.size() > 0) {
				util.log(titleList.get(0).getText());
			} else {
				util.log("*** No Title Found ***");
			}

			if (priceList.size() > 0) {
				util.log(priceList.get(0).getText());
			} else {
				util.log("*** No Price Found ***");
			}

			if (buyItNowList.size() > 0) {
				util.log(buyItNowList.get(0).getText());
			} else {
				util.log("*** No Buy details Found ***");
			}

			if (postageList.size() > 0) {
				util.log(postageList.get(0).getText());
			} else {
				util.log("*** No Postage details Found ***");
			}

			if (bidsList.size() > 0) {
				util.log(bidsList.get(0).getText());
			} else {
				util.log("*** No bids Found ***");
			}

			count = count + 1;
		}
		util.log("Items in the page : " + count);
	}
	...
	public void navigatePages(int i) {
		...
		util.log("Clicked on page number" + i);
	}
	/*
	 * @author:Navakanth
	...
	public void verifyPaginationDisplay() {
		if (paginationOptions.size() > 0) {
			util.log("Pagination is displayed");
		}
	}
